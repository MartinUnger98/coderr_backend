...
class TestOrderCRUD(APITestCase):
    def setUp(self):
        ...
        self.offer = Offer.objects.create(user=self.business_user, title="Design", description="Desc")
    ...
    def test_create_order(self):
        response = self.client.post('/api/orders/', self.order_payload, **self.customer_auth)
        self.assertEqual(response.status_code, 201)
        self.assertEqual(response.data["title"], self.offer_detail.title)

    def test_list_orders_as_customer(self):
        self.client.post('/api/orders/', self.order_payload, **self.customer_auth)
    ...
    def test_create_order_invalid_offer_detail_id(self):
        response = self.client.post('/api/orders/', {'offer_detail_id': 999}, **self.customer_auth)
    ...
    def test_admin_can_delete_order(self):
        ...
        admin_user = User.objects.create_superuser('admin', 'admin@mail.com', 'adminpass')
        Token.objects.create(user=admin_user)
        admin_auth = {'HTTP_AUTHORIZATION': f'Token {admin_user.auth_token.key}'}

        response = self.client.delete(f'/api/orders/{order_id}/delete/', **admin_auth)
    ...
    def test_non_admin_cannot_delete_order(self):
        ...
        response = self.client.delete(f'/api/orders/{order_id}/delete/', **self.customer_auth)
