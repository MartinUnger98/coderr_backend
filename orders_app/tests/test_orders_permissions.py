...
class TestOrderPermissions(APITestCase):
    ...
    def test_business_user_cannot_create_order(self):
        response = self.client.post('/api/orders/', self.order_payload, **self.business_auth)
        self.assertEqual(response.status_code, 403)

    def test_business_user_can_update_status(self):
        order = self.client.post('/api/orders/', self.order_payload, **self.customer_auth).data
        order_id = order['id']
