...
class OfferPermissionTests(APITestCase):
    def setUp(self):
        self.business_user = User.objects.create_user(username='biz', password='testpass')
        self.customer_user = User.objects.create_user(username='cust', password='testpass')
        self.business_profile = UserProfile.objects.create(user=self.business_user, username='biz', type='business')
        self.customer_profile = UserProfile.objects.create(user=self.customer_user, username='cust', type='customer')
    ...
    def test_only_creator_can_patch_offer(self):
        ...
        create_resp = self.client.post(reverse('offers-list'), self.offer_payload, format='json')
        offer_id = create_resp.data['id']
        
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.token_customer.key)
        url = reverse('offer-detail-update-delete', kwargs={'id': offer_id})
        patch_resp = self.client.patch(url, {'title': 'Hacker Title'}, format='json')
    ...
    def test_creator_can_patch_and_delete(self):
        ...
        url = reverse('offer-detail-update-delete', kwargs={'id': offer_id})
        patch_resp = self.client.patch(url, {'title': 'Neuer Titel'}, format='json')
