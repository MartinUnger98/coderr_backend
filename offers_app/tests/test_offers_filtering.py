...
class OfferFilteringTests(APITestCase):
    def setUp(self):
        ...
        self.user2 = User.objects.create_user(
            username='u2', password='testpass')
        UserProfile.objects.create(
            user=self.user1, username='u1', type='business')
        UserProfile.objects.create(
            user=self.user2, username='u2', type='business')
        Token.objects.create(user=self.user1)
        Token.objects.create(user=self.user2)

        self.offer1 = Offer.objects.create(
            user=self.user1, title='Logo Design', description='Professionell')
        self.offer2 = Offer.objects.create(
            user=self.user2, title='Flyer Design', description='Modern')

        OfferDetail.objects.create(offer=self.offer1, title='Basic', revisions=1,
                                   delivery_time_in_days=3, price=50, features=[], offer_type='basic')
        OfferDetail.objects.create(offer=self.offer1, title='Premium', revisions=5,
                                   delivery_time_in_days=5, price=150, features=[], offer_type='premium')

        OfferDetail.objects.create(offer=self.offer2, title='Basic', revisions=1,
                                   delivery_time_in_days=10, price=200, features=[], offer_type='basic')
    ...
    def test_filter_by_min_price(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTrue(all(float(o['min_price']) >=
                        100 for o in response.data['results']))

    def test_filter_by_max_delivery_time(self):
        url = reverse('offers-list') + "?max_delivery_time=5"
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTrue(all(o['min_delivery_time'] <=
                        5 for o in response.data['results']))
    ...
    def test_search_in_title_and_description(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTrue(any("Flyer" in o['title']
